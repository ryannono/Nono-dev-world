...
function removeDuplicates(nums) {
    let lastSeenDigit = nums[0], length = nums.length;
...
function removeDuplicates1(nums) {
    for (let i = 0; i < nums.length - 1; i++) {
        nums.splice(i, nums.reduce((prev, val) => prev + (val === nums[i] ? 1 : 0), -1));
    }
    return nums.length;
}
// ----------------- Main ----------------- //
...
testInputs.forEach((numberArray, testNumber) => {
    ...
    console.log(removeDuplicates(numberArray[0]) === numberArray[1]
        ? '\nVerification PASSED'
        : '\nVerification FAILED');
    console.log(removeDuplicates1(numberArray[0]) === numberArray[1]
        ? '\nVerification PASSED\n\n'
        : '\nVerification FAILED\n\n');
});
