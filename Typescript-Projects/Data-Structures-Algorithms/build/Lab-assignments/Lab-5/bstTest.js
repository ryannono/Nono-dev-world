 ...
 */
function insertionTest(tree, item) {
    if (item === undefined) {
 ...
 */
function searchTest(tree) {
    const repeatedSearch = (item) => {
        for (let i = 0; i < 100000; i++) {
            tree.search(item);
        }
    };
    let startTime;
    const timeFor1 = ((startTime = new Date().getTime()),
        repeatedSearch(1),
        new Date().getTime() - startTime);
    const timeFor15 = ((startTime = new Date().getTime()),
        repeatedSearch(15),
        new Date().getTime() - startTime);
 ...
 */
function removalTest(tree, item) {
    tree.delete(item);
    console.log(tree.inorderTraversal());
}
/**
