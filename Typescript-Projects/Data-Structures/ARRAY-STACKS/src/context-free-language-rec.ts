...
const allowedRecInputs = ['0', '1'] as const;

// --------------- Types to verify -------------- //
type allowedRecInput = (typeof allowedRecInputs)[number];
type zero = (typeof allowedRecInputs)[0];
 ...
 */
function isAllowedRecInput(input: string): input is allowedRecInput {
 ...
 */
function isZero(input: allowedRecInput): input is zero {
  return input === '0';
}
 ...
 * @returns A boolean value.
 */
function verifyLanguage(stack: languageStack, input: string) {
  for (let i = 0, length = input.length; i < length; i++) {
    const currentCharacter = input[i];
    if (
      !isAllowedRecInput(currentCharacter) ||
...
testRecInputs.forEach((testString, testNumber) => {
  ...
  console.log(
    verifyLanguage(langStack, testString)
