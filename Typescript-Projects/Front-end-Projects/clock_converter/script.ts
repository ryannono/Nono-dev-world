...
function timeConversion(input_value: string, select_value: string): string {
  ...
  let hour_digit_count = 0;
  for (let i = 0; input_value[i] !== ' '; i++) {
    hour_digit_count++;
  }
...
function valid_input_check(
  ...
  for (let spa_rep_count = 0; spa_rep_count < 4; spa_rep_count++) {
    input_value = input_value.replace(/\s/, '');
  }
  for (let col_rep_count = 0; col_rep_count < 2; col_rep_count++) {
    input_value = input_value.replace(':', '');
  }

  // get numerical value of parsed/reworked user input
  const input_val_num = Number(input_value);
  ...
  if (125959 >= input_val_num && input_val_num >= 0) {
    ...
    if (input_len === 12) {
      ...
      // return false and colour input box red
        ...
        select.style.backgroundColor = '#ff5656';
        console.log(
          'invalid input, number to low for regular time: ' +
            input_value +
            'becomes' +
            input_val_num
        );
    ...
    // keep input box white but return false
      ...
      select.style.backgroundColor = 'white';
      console.log(
        "invalid input full time hasn't be entered yet: " +
          input_value +
          'becomes' +
          input_val_num
      );
...
function assign_value(
  ...
  if (valid_input_check(input, select)) {
    const converted_input_value: string = timeConversion(
      input_value,
      select_value
    );
 ...
 */
function auto_format(input: HTMLInputElement): void {
  const input_value: string = input.value;
  const input_len: number = input_value.length;
...
input.addEventListener('emptied', () => valid_input_check(input, select));

// on button click the value in the input field gets converted
// and assigned to the heading
const button = document.getElementById('converter');
button.addEventListener('click', () => assign_value(input, select));
