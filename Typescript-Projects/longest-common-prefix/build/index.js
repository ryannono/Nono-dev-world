 ...
 */
function goodFilterResult(strs, longestPrefix, fullLength) {
    return (strs.filter(string => string.startsWith(strs[0].substring(0, longestPrefix.length + 1))).length === fullLength && longestPrefix.length < strs[0].length);
}
/**
...
testInputs.forEach((stringArray, testNumber) => {
    ...
    console.log(longestCommonPrefix(stringArray[0]) === stringArray[1]
        ? '\nVerification PASSED\n\n'
        : '\nVerification FAILED\n\n');
});
