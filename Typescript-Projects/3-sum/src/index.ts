 ...
 */
function isHorizontallyCorrect(sudokuLine: string[]) {
 ...
 * @returns A boolean value.
 */
function isVerticallyCorrect(sudokuBoard: string[][], columnToCheck: number) {
  const verticalSet = new Set<string>();
  for (const row of sudokuBoard) {
    const currentElement = row[columnToCheck];
    if (verticalSet.has(currentElement)) return false;
    if (currentElement !== '.') verticalSet.add(currentElement);
  }
  return true;
}
...
function generateSubgrid(
  ...
  while (subgrid.length < 9) {
    subgrid.push(
      board[columnIndex][rowIndex + subgrid.length / 3],
      board[columnIndex + 1][rowIndex + subgrid.length / 3],
      board[columnIndex + 2][rowIndex + subgrid.length / 3]
    );
  }
  return subgrid;
}
 ...
 * @returns A boolean value.
 */
function subgridsAreCorrect(board: string[][]) {
  const subgridSet = new Set();
  for (
    let columnIndex = 0, numColumn = board.length;
    columnIndex < numColumn;
    columnIndex += 3
  ) {
    for (
      let rowIndex = 0, numRows = numColumn;
      rowIndex < numRows;
      rowIndex += 3
    ) {
      const subgrid = generateSubgrid(board, columnIndex, rowIndex).filter(
        val => {
          if (val !== '.') {
            subgridSet.add(val);
            return true;
      ...
      if (subgrid.length !== subgridSet.size) return false;
      subgridSet.clear();
    }
  }
  return true;
...
function isValidSudoku(board: string[][]): boolean {
  for (const row of board) {
    if (!isHorizontallyCorrect(row)) return false;
  }

  for (
    let columnIndex = 0, numColumns = board[0].length;
    columnIndex < numColumns;
    columnIndex++
  ) {
    if (!isVerticallyCorrect(board, columnIndex)) return false;
  }

  return subgridsAreCorrect(board);
}

const testInputs = new Map<number, [string[][], boolean]>([
  [
    1,
    [
      [
...
testInputs.forEach((testInput, key) => {
  ...
  console.log(
    `Result: ${
      isValidSudoku(testInput[0]) === testInput[1] ? 'Passed\n' : 'Failed\n'
    }`
  );
});
