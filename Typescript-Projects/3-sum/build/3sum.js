...
// -------------- quickSort ------------- //
function insertionSort(array, unsortedStart = 1) {
...
function threeSum(nums) {
    const confirmedTriplets = [];
    const fullLength = nums.length;
    quickSort(nums, 0, fullLength - 1);
    let num1, num2, num3, sum;
    for (let num1Index = 0, length = fullLength - 2; (num1 = nums[num1Index]) * 3 <= 0 && num1Index < length; num1Index++) {
        if (num1Index === 0 || num1 !== nums[num1Index - 1]) {
            for (let num2Index = num1Index + 1, length = fullLength - 1; num1 + (num2 = nums[num2Index]) * 2 <= 0 && num2Index < length; num2Index++) {
                if (num2Index === num1Index + 1 || num2 !== nums[num2Index - 1]) {
                    for (let num3Index = num2Index + 1, length = fullLength; num3Index < length; num3Index++) {
                        num3 = nums[num3Index];
                        if (num3Index === num2Index + 1 || num3 !== nums[num3Index - 1]) {
                            sum = num1 + num2 + num3;
                            if (sum === 0)
                                confirmedTriplets.push([num1, num2, num3]);
                            if (sum > 0)
                                break;
                        }
                    }
